...
def check_llm_api_key() -> tuple[bool, str]:
    ...
    Reads from os.environ at call time so that test patches take effect.
    """
    provider = os.getenv("BLOP_LLM_PROVIDER", BLOP_LLM_PROVIDER).lower()
    if provider == "anthropic":
        return (bool(os.getenv("ANTHROPIC_API_KEY", ANTHROPIC_API_KEY)), "ANTHROPIC_API_KEY")
    if provider == "openai":
        return (bool(os.getenv("OPENAI_API_KEY", OPENAI_API_KEY)), "OPENAI_API_KEY")
    return (bool(os.getenv("GOOGLE_API_KEY", GOOGLE_API_KEY)), "GOOGLE_API_KEY")
